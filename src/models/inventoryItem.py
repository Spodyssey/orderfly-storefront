...
class InventoryItemDAO:
    ...
    def create(self, inventory):
        insert_query = '''
            INSERT OR REPLACE INTO inventory_item (inventory_uuid, item_asin)
            VALUES (?, ?)
        '''
    ...
    def read(self, inventory_uuid):
        select_query = '''
            SELECT * FROM inventory_item WHERE inventory_uuid = ?
        '''
        ...
        if rows:
            for row in rows:
                ...
                inventory_item.append(InventoryItem(inventory_uuid, item_asin))    
    ...
    #     update_query = '''
    #         UPDATE inventory_item SET last_updated_date = ? WHERE id = ? AND marketplace_id = ?
    #     '''
    ...
    def delete(self, inventory):
        delete_query = '''
            DELETE FROM inventory_item WHERE inventory_uuid = ?
        '''
